...
import { CameraImage } from './../../../models/LSGAMEPIPAD/CameraImage';
...
export class CameraImageService {
   ...
   url = environment.api + 'cameraimage/';
   ...
   post(cameraimage: CameraImage): Promise<any> {
      return this.http.post(this.url, JSON.stringify(cameraimage), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(cameraimage: CameraImage): Promise<any> {
      return this.http.put(this.url, JSON.stringify(cameraimage), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
